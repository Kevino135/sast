...
def print_results(printJson, issue):
    ...
    # addition
    stringsFound = issue['stringsFound']
    author = issue['author']
...
#            print(printableDiff)
            # addition
            print("{}Strings Found:".format(bcolors.OKGREEN))
            for strings in stringsFound:
                resultStr = "{}{}{}".format(bcolors.WARNING, strings, bcolors.ENDC)
                print(resultStr)
         else:
...
def regex_check(printableDiff, commit_time, branch_name, prev_commit, blob, commitHash, custom_regexes={}):
    ...
    # ini jadi pr, buat agar simpen semua yg match
    for key in secret_regexes:
        ...
        if found_strings:
            ...
            # addition
            foundRegex['author'] = prev_commit.author.name + ' <' + prev_commit.author.email + '>'
